        ...
        if (msg == '1'):
            ...
            for tweet in api.user_timeline():
                if (tweet.in_reply_to_status_id == None):
                    ...
                    x = re.findall('(https?://[^\\s]+)', stringtoadd)
                    if (len(x) != 0):
            ...
            for tweet in api.user_timeline():
                if (tweet.in_reply_to_status_id != None):
                    stringtoadd = '{}. Reply To  *@{}*({}) :\n {}\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t{}\n\n\t*🔃 : {}\t\t\t\t\t💟 : {}*\n\n------------------------------------\n\n'.format(timelinecount, tweet.entities['user_mentions'][0]['screen_name'], tweet.entities['user_mentions'][0]['name'], tweet.text, tweet.created_at.strftime('   (%b %d, %H:%M)'), tweet.retweet_count, (tweet.retweeted_status.favorite_count if (tweet.retweeted == True) else tweet.favorite_count))
                    x = re.findall('(https?://[^\\s]+)', stringtoadd)
                    if (len(x) != 0):
                        stringtoadd = stringtoadd.replace(x[0], '\n{}'.format(x[0]))
                    if ((len(timeline) + len(stringtoadd)) > 1500):
            ...
            for ms in msgs:
                ...
                if (ms.message_create['target']['recipient_id'] == user_id):
                    if (ms.message_create['sender_id'] in msgdict.keys()):
                        ...
                        msgdict[ms.message_create['sender_id']].append(tempchat.copy())
                elif (ms.message_create['target']['recipient_id'] in msgdict.keys()):
