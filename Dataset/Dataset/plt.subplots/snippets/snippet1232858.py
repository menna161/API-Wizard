...
def plot_objective(result, param_thr, hyper_param_names, par_cnt_scheme='enc_dec_cnn_lstm_ff', levels=10, n_points=20, n_samples=250, size=2, zscale='linear', selected_dimensions=None, dimensions=None):
    ...
    for (i, dim_i) in enumerate(selected_dimensions):
        for (j, dim_j) in enumerate(selected_dimensions):
            if (i == j):
                ...
                (xi, yi, zi) = partial_dependence_valid_samples_allow_paramcounts(space, result.models[(- 1)], param_thr, hyper_param_names, dim_i, dim_j, par_cnt_scheme, n_samples=n_samples, n_points=n_points)
    ...
    complete_ax = _format_scatter_plot_axes(ax, space, ylabel='Partial dependence', param_thr=param_thr, selected_dimensions=selected_dimensions, dim_labels=dimensions)
