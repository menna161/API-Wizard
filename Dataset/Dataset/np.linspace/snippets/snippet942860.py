...
def donut(inner_r, outer_r, height=5, point_density=24, n_viewpoints=60, offset=0.001):
    assert isinstance(height, int)
    assert (outer_r > inner_r)

    def round_points(thetas):
        return np.vstack([np.cos(thetas), np.zeros(thetas.shape[0]), np.sin(thetas)]).T
    ...
    camera_r = ((inner_r + outer_r) / 2.0)
    camera_y = ((point_ys[0] + point_ys[(- 1)]) / 2.0)
    ...
    camera_omegas = np.vstack((np.zeros(n_viewpoints), (- thetas), np.zeros(n_viewpoints))).T
