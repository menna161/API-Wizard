...
def linear_interpolate_between_inputs(self, checkpoint, start_sent, end_sent, num_samples=8):
    ...
    end_idx_seq = ([self.word_index.get(word, self.unk) for word in end_sent] + [self.eos])
    start_idx_seq = np.concatenate([start_idx_seq, np.zeros(max(0, (self.num_tokens - len(start_idx_seq))))])[:self.num_tokens]
    end_idx_seq = np.concatenate([end_idx_seq, np.zeros(max(0, (self.num_tokens - len(end_idx_seq))))])[:self.num_tokens]
    ...
    with tf.Session() as sess:
        ...
        result = sess.run(self.inference_logits, feed_dict={self.z_vector: sampled, self.keep_prob: 1.0})
        for (i, pred) in enumerate(result[:num_samples]):
            print('G: {}'.format(' '.join([self.idx_word[i] for i in pred if (i not in [self.pad, self.eos])])))
