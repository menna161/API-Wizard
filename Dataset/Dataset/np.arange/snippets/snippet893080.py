...
def fill(self):
    ...
    for (i_index_im, index_im) in enumerate(indices_chunk):
        (self._fixed_im_list[i_index_im], self._deformed_im_list[i_index_im], self._dvf_list[i_index_im], torso_list[i_index_im]) = ag.get_dvf_and_deformed_images(self._setting, im_info=im_info_list_full[index_im], stage=self._stage, mode_synthetic_dvf=self._mode_artificial_generation)
        ...
        if self._setting['verbose']:
            ...
            if (self._class_mode == 'Thread'):
                with open(su.address_generator(self._setting, 'log_im_file'), 'a+') as f:
                    f.write((log_msg + '\n'))
    ishuffled_folder = reading_utils.get_ishuffled_folder_write_ishuffled_setting(self._setting, self._train_mode, self._stage, self._number_of_images_per_chunk, self._samples_per_image, self._im_info_list_full, full_image=self._full_image)
    ...
    if os.path.isfile(ishuffled_address):
        ...
        self._ishuffled = reading_utils.shuffled_indices_from_chunk(self._setting, dvf_list=self._dvf_list, torso_list=torso_list, im_info_list=im_info_list, stage=self._stage, semi_epoch=self._semi_epoch, chunk=self._chunk, samples_per_image=self._samples_per_image, log_header=self._class_mode, full_image=self._full_image)
