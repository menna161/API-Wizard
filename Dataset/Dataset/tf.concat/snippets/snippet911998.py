...
def embedding_layer(self):
    ...
    with tf.name_scope('decoder_inputs'):
        keep = tf.where((tf.random_uniform([self.batch_size, self.num_tokens]) < self.word_dropout_keep_prob), tf.fill([self.batch_size, self.num_tokens], True), tf.fill([self.batch_size, self.num_tokens], False))
        ...
        self.dec_input = tf.concat([tf.fill([self.batch_size, 1], self.word_index['GO']), ending], 1, name='dec_input')
