...
def rnn(images, real_input_flag, num_layers, num_hidden, configs):
    ...
    with tf.variable_scope('generator'):
        ...
        for time_step in range((window_length - 1)):
            input_list.append(tf.zeros([batch_size, ims_width, ims_height, output_channels]))
        for time_step in range((total_length - 1)):
            with tf.variable_scope('e3d-lstm', reuse=reuse):
                if (time_step < input_length):
                    ...
                    input_frm = ((real_input_flag[(:, time_diff)] * images[(:, time_step)]) + ((1 - real_input_flag[(:, time_diff)]) * x_gen))
                ...
                x_gen = tf.layers.conv3d(hidden[(num_layers - 1)], output_channels, [window_length, 1, 1], [window_length, 1, 1], 'same')
