...
def jwt_login(consumer_id, username, private_key, api_version, sandbox=False):
    ...
    jwt_payload = jwt.encode({'exp': (datetime.datetime.utcnow() + datetime.timedelta(seconds=30)), 'iss': consumer_id, 'aud': endpoint, 'sub': username}, private_key, algorithm='RS256')
    ...
    return simple_salesforce.Salesforce(instance_url=body['instance_url'], session_id=body['access_token'], version=api_version)
