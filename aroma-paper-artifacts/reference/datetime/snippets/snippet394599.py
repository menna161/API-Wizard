...
def test_snowflake_materialization_consistency_internal():
    snowflake_config = IntegrationTestRepoConfig(online_store=SNOWFLAKE_ONLINE_CONFIG, offline_store_creator=SnowflakeDataSourceCreator, batch_engine=SNOWFLAKE_ENGINE_CONFIG)
    snowflake_environment = construct_test_environment(snowflake_config, None)
    df = create_basic_driver_dataset()
    ds = snowflake_environment.data_source_creator.create_data_source(df, snowflake_environment.feature_store.project, field_mapping={'ts_1': 'ts'})
    fs = snowflake_environment.feature_store
    driver = Entity(name='driver_id', join_keys=['driver_id'])
    driver_stats_fv = FeatureView(name='driver_hourly_stats', entities=[driver], ttl=timedelta(weeks=52), source=ds)
    try:
