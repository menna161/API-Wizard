...
def run(args):
    ...
    try:
        ...
        permissions = None
    if session_uid:
        login_session = LoginSession.load(uid=session_uid, scope=scope, permissions=permissions)
    else:
        if (short_uid is None):
            raise PermissionError('You must specify either the session_uid or the short_uid of the login session')
        try:
            status = args['status']
        except:
            raise PermissionError('You must specify the status of the short_uid session you wish to query...')
        login_session = LoginSession.load(short_uid=short_uid, status=status, scope=scope, permissions=permissions)
    return_value = {}
    ...
    if login_session.is_approved():
        ...
        should_return_data = True
        return_value['logout_datetime'] = datetime_to_string(login_session.logout_time())
    if should_return_data:
        return_value['public_cert'] = login_session.public_certificate().to_data()
