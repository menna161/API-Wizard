...
def plot_create2_mover(env, batch_size, shared_returns, plot_running):
    ...
    ax1_bars = ax1.bar(list(range((sensor_array_dim + action_array_dim))), ([0] * (sensor_array_dim + action_array_dim)), tick_label=(list(sensor_array_type.names) + ['requested left wheel speed', 'requested right wheel speed']))
    ax1_min_y = [create2_config.PACKET_INFO[create2_config.PACKET_NAME_TO_ID[packet]]['range'][0] for packet in sensor_array_type.names]
    ax1_min_y.extend([r[0] for r in create2_config.OPCODE_INFO[create2_config.OPCODE_NAME_TO_CODE[env._main_op]]['params'].values()])
    ax1_max_y = [create2_config.PACKET_INFO[create2_config.PACKET_NAME_TO_ID[packet]]['range'][1] for packet in sensor_array_type.names]
    ax1_max_y.extend([r[1] for r in create2_config.OPCODE_INFO[create2_config.OPCODE_NAME_TO_CODE[env._main_op]]['params'].values()])
    ax1.set_ylim([(- 1.25), 1.25])
    ax1.tick_params(axis='x', labelrotation=10, labelsize=7)
    ax1.set_title('Current Sensor & Action')
    ax1_texts = []
    for b in ax1_bars:
        ax1_texts.append(ax1.text((b.get_x() + (b.get_width() / 2.0)), (1.05 * b.get_height()), '0', ha='center', va='bottom'))
    (hl11,) = ax2.plot([], [])
    ...
    ax3_texts = []
    for b in ax3_bars:
        ax3_texts.append(ax3.text((b.get_x() + (b.get_width() / 2.0)), (1.05 * b.get_height()), '0', ha='center', va='bottom'))
    ...
    while plot_running.value:
        ...
        for (i, b) in enumerate(ax1_bars):
            ...
            ax1_texts[i] = ax1.text((b.get_x() + (b.get_width() / 2.0)), (1.05 * b.get_height()), '{:.2f}'.format(raw_val), ha='center', va='bottom')
        for (i, b) in enumerate(ax3_bars):
            ...
            b.set_height(max((- 1.0), min(1.0, height)))
            ax3_texts[i] = ax3.text((b.get_x() + (b.get_width() / 2.0)), (1.05 * b.get_height()), '{:.2f}'.format(raw_val), ha='center', va='bottom')
