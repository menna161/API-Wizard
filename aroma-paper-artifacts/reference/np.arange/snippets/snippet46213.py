...
def test(self, evaluation=True, iter_time=0, save_motion=False):
    ...
    actions = ['walking', 'eating', 'smoking', 'discussion', 'directions', 'greeting', 'phoning', 'posing', 'purchases', 'sitting', 'sittingdown', 'takingphoto', 'waiting', 'walkingdog', 'walkingtogether']
    self.io.print_log(' ')
    ...
    for (action_num, action) in enumerate(actions):
        (encoder_inputs, decoder_inputs, targets, downsample_idx) = srnn_sample(normed_test_dict, action, self.arg.source_seq_len, self.arg.target_seq_len, len(self.dim_use))
        ...
        if evaluation:
            ...
            self.io.print_log(print_str)
    self.io.print_log(' ')
    self.mean_MAE_select[iter_time] = np.mean(((((self.MAE_tensor[(iter_time, :, 1)] + self.MAE_tensor[(iter_time, :, 3)]) + self.MAE_tensor[(iter_time, :, 7)]) + self.MAE_tensor[(iter_time, :, 9)]) / 4))
