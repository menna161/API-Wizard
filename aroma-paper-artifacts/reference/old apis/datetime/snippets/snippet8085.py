...
def execute(self, cheque):
    ...
    from Acquire.Client import Account as _Account
    access_account = _Account(user=access_user, account_uid=account_uid, accounting_service=accounting_service)
    try:
        credit_notes = cheque.cash(spend=self.total_cost(), resource=self.request().fingerprint())
    except Exception as e:
        from Acquire.Service import exception_to_string
        from Acquire.Accounting import PaymentError
        raise PaymentError(('Problem cashing the cheque used to pay for the calculation: \n\nCAUSE: %s' % exception_to_string(e)))
    ...
    from Acquire.ObjectStore import get_datetime_future as _get_datetime_future
    endtime = _get_datetime_future(days=2)
    self.save()
    compute_cheque = _Cheque.write(account=access_account, resource=('work %s' % self.uid()), max_spend=10.0, recipient_url=compute_service.canonical_url(), expiry_date=endtime)
    storage_cheque = _Cheque.write(account=access_account, resource=('work %s' % self.uid()), max_spend=10.0, recipient_url=storage_service.canonical_url(), expiry_date=endtime)
    ...
    rule = _ACLUserRules.owner(user_guid=access_user.guid()).add(user_guid=self.user_guid(), rule=_ACLRule.reader())
    aclrules = _ACLRules(rule=rule, default_rule=_ACLRule.denied())
    output_drive = _Drive(name=('output_%s' % self.uid()), creds=creds, aclrules=aclrules, cheque=storage_cheque, max_size='10MB', autocreate=True)
    self._output_loc = output_drive.metadata().location()
    self._status = 'awaiting (paid, have drive)'
    self.save()
    from Acquire.Client import PAR as _PAR
    par = _PAR(location=self._output_loc, user=access_user, aclrule=_ACLRule.writer(), expires_datetime=endtime)
    secret = compute_service.encrypt_data(par.secret())
    args = {'worksheet_uid': self.uid(), 'request': self.request().to_data(), 'par': par.to_data(), 'secret': secret, 'cheque': compute_cheque.to_data()}
