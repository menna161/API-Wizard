...
def dusk(observer: Observer, date: Optional[datetime.date]=None, depression: Union[(float, Depression)]=Depression.CIVIL, tzinfo: Union[(str, datetime.tzinfo)]=datetime.timezone.utc) -> datetime.datetime:
    ...
    if isinstance(depression, Depression):
        ...
        dep = depression
    try:
        tot = time_of_transit(observer, date, (90.0 + dep), SunDirection.SETTING).astimezone(tzinfo)
        tot_date = tot.date()
        if (tot_date != date):
            if (tot_date < date):
                ...
                delta = datetime.timedelta(days=(- 1))
            new_date = (date + delta)
            tot = time_of_transit(observer, new_date, (90.0 + dep), SunDirection.SETTING).astimezone(tzinfo)
            tot_date = tot.date()
            if (tot_date != date):
