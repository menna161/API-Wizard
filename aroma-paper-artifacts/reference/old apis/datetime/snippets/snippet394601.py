...
def test_spark_materialization_consistency():
    spark_config = IntegrationTestRepoConfig(provider='local', online_store_creator=RedisOnlineStoreCreator, offline_store_creator=SparkDataSourceCreator, batch_engine={'type': 'spark.engine', 'partitions': 10})
    spark_environment = construct_test_environment(spark_config, None, entity_key_serialization_version=1)
    df = create_basic_driver_dataset()
    ds = spark_environment.data_source_creator.create_data_source(df, spark_environment.feature_store.project, field_mapping={'ts_1': 'ts'})
