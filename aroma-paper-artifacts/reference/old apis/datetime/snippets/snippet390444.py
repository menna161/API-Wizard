...
def generate_auth_cert(domain, base_year, quiet=False):
    ...
    builder = CertificateBuilder({'country_name': 'US', 'state_or_province_name': 'Massachusetts', 'locality_name': 'Newbury', 'organization_name': 'Bad TLS Limited', 'common_name': 'required-auth.{}'.format(domain)}, public_key)
    builder.issuer = ca_cert
    builder.subject_alt_domains = ['required-auth.{}'.format(domain), 'optional-auth.{}'.format(domain)]
