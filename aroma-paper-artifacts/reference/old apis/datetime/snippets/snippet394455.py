...
def test_apply_with_fv_inference() -> None:
    ...
    with tempfile.TemporaryDirectory() as data_dir:
        ...
        with runner.local_repo(get_example_repo('example_feature_repo_with_feature_service_3.py').replace('%PARQUET_PATH%', driver_stats_path).replace('%PARQUET_PATH_GLOBAL%', global_stats_path), 'file') as store:
