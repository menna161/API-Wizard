...
parser.add_argument('--SE_file', default='data/SE(METR).txt', help='spatial emebdding file')
parser.add_argument('--model_file', default='data/GMAN(METR)', help='save the model to disk')
parser.add_argument('--log_file', default='data/log(METR)', help='log file')
...
bn_momentum = tf.compat.v1.train.exponential_decay(0.5, global_step, decay_steps=((args.decay_epoch * num_train) // args.batch_size), decay_rate=0.5, staircase=True)
...
learning_rate = tf.compat.v1.train.exponential_decay(args.learning_rate, global_step, decay_steps=((args.decay_epoch * num_train) // args.batch_size), decay_rate=0.7, staircase=True)
learning_rate = tf.maximum(learning_rate, 1e-05)
...
for epoch in range(args.max_epoch):
    ...
    for batch_idx in range(num_batch):
        ...
        feed_dict = {X: trainX[start_idx:end_idx], TE: trainTE[start_idx:end_idx], label: trainY[start_idx:end_idx], is_training: True}
    ...
    for batch_idx in range(num_batch):
        ...
        feed_dict = {X: valX[start_idx:end_idx], TE: valTE[start_idx:end_idx], label: valY[start_idx:end_idx], is_training: False}
        loss_batch = sess.run(loss, feed_dict=feed_dict)
        val_loss += (loss_batch * (end_idx - start_idx))
    val_loss /= num_val
    end_val = time.time()
    utils.log_string(log, ('%s | epoch: %04d/%d, training time: %.1fs, inference time: %.1fs' % (datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), (epoch + 1), args.max_epoch, (end_train - start_train), (end_val - start_val))))
    utils.log_string(log, ('train loss: %.4f, val_loss: %.4f' % (train_loss, val_loss)))
...
for batch_idx in range(num_batch):
    start_idx = (batch_idx * args.batch_size)
    end_idx = min(num_train, ((batch_idx + 1) * args.batch_size))
    feed_dict = {X: trainX[start_idx:end_idx], TE: trainTE[start_idx:end_idx], is_training: False}
...
for batch_idx in range(num_batch):
    start_idx = (batch_idx * args.batch_size)
    end_idx = min(num_val, ((batch_idx + 1) * args.batch_size))
    feed_dict = {X: valX[start_idx:end_idx], TE: valTE[start_idx:end_idx], is_training: False}
    pred_batch = sess.run(pred, feed_dict=feed_dict)
...
for batch_idx in range(num_batch):
    ...
    feed_dict = {X: testX[start_idx:end_idx], TE: testTE[start_idx:end_idx], is_training: False}
    pred_batch = sess.run(pred, feed_dict=feed_dict)
...
for q in range(args.Q):
    ...
    RMSE.append(rmse)
    MAPE.append(mape)
    utils.log_string(log, ('step: %02d         %.2f\t\t%.2f\t\t%.2f%%' % ((q + 1), mae, rmse, (mape * 100))))
average_mae = np.mean(MAE)
average_rmse = np.mean(RMSE)
average_mape = np.mean(MAPE)
utils.log_string(log, ('average:         %.2f\t\t%.2f\t\t%.2f%%' % (average_mae, average_rmse, (average_mape * 100))))
end = time.time()
