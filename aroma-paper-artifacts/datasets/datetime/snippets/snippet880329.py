...
def delete_organization_invitation_with_http_info(self, owner: Annotated[(StrictStr, Field(..., description='Owner of the namespace'))], member_user: Annotated[(Optional[StrictStr], Field(description='User.'))]=None, member_user_email: Annotated[(Optional[StrictStr], Field(description='Read-only User email.'))]=None, member_role: Annotated[(Optional[StrictStr], Field(description='Role.'))]=None, member_kind: Annotated[(Optional[StrictStr], Field(description='Kind.'))]=None, member_created_at: Annotated[(Optional[datetime], Field(description='Optional time when the entity was created.'))]=None, member_updated_at: Annotated[(Optional[datetime], Field(description='Optional last time the entity was updated.'))]=None, email: Annotated[(Optional[StrictStr], Field(description='Optional email.'))]=None, **kwargs):
    ...
    for (_key, _val) in _params['kwargs'].items():
        if (_key not in _all_params):
            raise ApiTypeError(("Got an unexpected keyword argument '%s' to method delete_organization_invitation" % _key))
        _params[_key] = _val
    del _params['kwargs']
    _collection_formats = {}
    _path_params = {}
    ...
    _response_types_map = {}
    return self.api_client.call_api('/api/v1/orgs/{owner}/invitations', 'DELETE', _path_params, _query_params, _header_params, body=_body_params, post_params=_form_params, files=_files, response_types_map=_response_types_map, auth_settings=_auth_settings, async_req=_params.get('async_req'), _return_http_data_only=_params.get('_return_http_data_only'), _preload_content=_params.get('_preload_content', True), _request_timeout=_params.get('_request_timeout'), collection_formats=_collection_formats, _request_auth=_params.get('_request_auth'))
