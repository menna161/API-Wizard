...
def test_entry_customer_out_of_hours(self):
    ...
    entry = create_entry_from_delta(dateDelta, time1, duration, 'trading hours test 1')
    ...
    entry.time = time2
    self.assertRaisesMessage(ValidationError, 'Sorry, the store is closed then. Try changing the time.', entry.clean)
