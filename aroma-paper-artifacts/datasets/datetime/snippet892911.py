...
def run_regnet(setting, stage=None):
    ...
    load_model = False
    network_name = 'crop4_connection'
    setting['stage'] = 1
    setting['AGMode'] = 'Resp'
    setting['use_keras'] = False
    deform_exp_s1 = '3D_max7_D14_K'
    setting['start_chunk'] = 0
    if (stage is not None):
        setting['stage'] = stage
    if load_model:
        loaded_network = {'NetworkLoad': '20190304_3D_max15_D14_K_NoResp_S2_r5_dec4', 'GlobalStepLoad': 'Last', 'BatchSize': 'Auto', 'itr_load': 'Auto', 'semi_epoch_load': 'Auto'}
        ...
        itr_load = 0
        semi_epoch_load = 0
    setting['DetailedNetworkSummary'] = False
    regularization = True
    inspecting_regularization_weight = False
    setting['ParallelGeneration1stEpoch'] = True
    setting['ParallelSearching'] = True
    setting['Randomness'] = True
    setting['PadTo'] = {('stage' + str(setting['stage'])): getattr(getattr(network, network_name), 'get_padto')()}
    (dsmoothlist_training, dsmoothlist_validation) = su.dsmoothlist_by_deform_exp(deform_exp_s1, setting['AGMode'])
    data_exp_dict = [{'data': 'SPREAD', 'deform_exp': deform_exp_s1, 'TrainingCNList': [i for i in range(1, 11)], 'TrainingTypeImList': [0, 1], 'TrainingDSmoothList': su.repeat_dsmooth_numbers(dsmoothlist_training, deform_exp_s1, repeat=2), 'TrainingDeformedImExt': ['Clean', 'Sponge', 'Noise'], 'ValidationCNList': [11, 12], 'ValidationTypeImList': [0, 1], 'ValidationDSmoothList': dsmoothlist_validation, 'ValidationDeformedImExt': ['Clean', 'Sponge', 'Noise']}, {'data': 'DIR-Lab_COPD', 'deform_exp': deform_exp_s1, 'TrainingCNList': [i for i in range(1, 10)], 'TrainingTypeImList': [0, 1], 'TrainingDSmoothList': su.repeat_dsmooth_numbers(dsmoothlist_training, deform_exp_s1, repeat=2), 'TrainingDeformedImExt': ['Clean', 'Sponge', 'Noise'], 'ValidationCNList': [10], 'ValidationTypeImList': [0, 1], 'ValidationDSmoothList': dsmoothlist_validation, 'ValidationDeformedImExt': ['Clean', 'Sponge', 'Noise']}]
    if (setting['stage'] == 1):
    ...
    setting['ClassBalanced'] = su.load_suggested_class_balanced(setting)
    if (setting['PadTo'][('stage' + str(setting['stage']))] is None):
        setting['FullImage'] = False
    else:
    ...
    setting['NetworkTraining']['LearningRate_Base'] = 0.001
    setting['NetworkTraining']['LearningRate_Decay'] = None
    setting['NetworkTraining']['MaxIterations'] = np.int(10000000.0)
    setting['NetworkValidation'] = dict()
    setting['NetworkValidation']['MaxQueueSize'] = 10
    if (setting['stage'] == 1):
        setting['NetworkTraining']['NumberOfImagesPerChunk'] = 16
    ...
    if (not semi_epoch_load):
        if setting['ParallelGeneration1stEpoch']:
            generate_1st_epoch = reading.chunk_image.Images(setting=setting, class_mode='1stEpoch', train_mode='Training', number_of_images_per_chunk=setting['NetworkTraining']['NumberOfImagesPerChunk'], samples_per_image=setting['NetworkTraining']['SamplesPerImage'], im_info_list_full=su.get_im_info_list_from_train_mode(setting, train_mode='Training'), stage=setting['stage'], semi_epoch=semi_epoch_load, full_image=setting['FullImage'])
    ...
    dvf_predict = getattr(getattr(network, setting['NetworkDesign']), 'network')(images_tf, bn_training, detailed_summary=setting['DetailedNetworkSummary'], use_keras=setting['use_keras'])
    ...
    if load_model:
        ...
        saver_loading.restore(sess, su.address_generator(setting, 'saved_model_with_step', current_experiment=loaded_network['NetworkLoad'], step=loaded_network['GlobalStepLoad']))
    ...
    for itr in range(itr_load, setting['NetworkTraining']['MaxIterations']):
        ...
        if (setting['NetworkTraining']['LearningRate_Decay'] is not None):
            lr = (setting['NetworkTraining']['LearningRate_Base'] * (setting['NetworkTraining']['LearningRate_Decay'] ** train_queue._chunk_direct._semi_epoch))
        ...
        if regularization:
            [loss_train_itr, huber_train_itr, bending_train_itr, _] = sess.run([loss, huber, bending_energy, train_step], feed_dict={images_tf: batch_im, dvf_ground_truth: batch_dvf, bn_training: 1, learning_rate: lr})
        else:
            [loss_train_itr, _] = sess.run([loss, train_step], feed_dict={images_tf: batch_im, dvf_ground_truth: batch_dvf, bn_training: 1, learning_rate: lr})
        ...
        if ((itr % 25) == 1):
            ...
            if regularization:
                ...
                [s, dvf_plot, dvf_predict_plot] = sess.run([summ, dvf_ground_truth, dvf_predict], feed_dict={images_tf: batch_im, dvf_ground_truth: batch_dvf, bn_training: 0, loss_average_tf: loss_train_average, huber_average_tf: huber_train_average, bending_average_tf: bending_train_average})
                logging.info((setting['current_experiment'] + ', itr={} SemiEpoch={}, loss={:.2f} , huber={:.2f}, bendingE={:.2f}'.format(itr, train_queue._chunk_direct._semi_epoch, loss_train_average, huber_train_average, bending_train_average)))
                huber_train_average = 0
                bending_train_average = 0
            else:
                [s, dvf_plot, dvf_predict_plot] = sess.run([summ, dvf_ground_truth, dvf_predict], feed_dict={images_tf: batch_im, dvf_ground_truth: batch_dvf, bn_training: 0, loss_average_tf: loss_train_average})
        ...
        if ((itr % 1000) == 1):
            ...
            while (not end_of_validation):
                ...
                if regularization:
                    [loss_val_itr, huber_val_itr, bending_val_itr] = sess.run([loss, huber, bending_energy], feed_dict={images_tf: batch_im_val, dvf_ground_truth: batch_dvf_val, bn_training: 0})
            ...
            if regularization:
                ...
                [dvf_plot_val, dvf_predict_plot_val, s] = sess.run([dvf_ground_truth, dvf_predict, summ], feed_dict={images_tf: batch_im_val, dvf_ground_truth: batch_dvf_val, bn_training: 0, loss_average_tf: loss_val_average, huber_average_tf: huber_val_average, bending_average_tf: bending_val_average})
            else:
                [dvf_plot_val, dvf_predict_plot_val, s] = sess.run([dvf_ground_truth, dvf_predict, summ], feed_dict={images_tf: batch_im_val, dvf_ground_truth: batch_dvf_val, bn_training: 0, loss_average_tf: loss_val_average})
