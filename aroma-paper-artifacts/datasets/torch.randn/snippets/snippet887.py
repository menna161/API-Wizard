...
def do_train(self, paths, dataset, optimiser, epochs, batch_size, step, lr=0.001, valid_index=[], use_half=False, do_clip=False):
    if use_half:
        import apex
    ...
    for e in range(epochs):
        trn_loader = DataLoader(dataset, collate_fn=(lambda batch: env.collate_emo_samples(pad_left, window, pad_right, batch)), batch_size=batch_size, num_workers=2, shuffle=True, pin_memory=True)
        start = time.time()
        running_loss_c = 0.0
        running_loss_f = 0.0
        running_loss_vq = 0.0
        ...
        running_entropy = 0.0
        running_loss_vq_f0 = 0.0
        running_loss_vqc_f0 = 0.0
        running_entropy_f0 = 0.0
