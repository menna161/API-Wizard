...
from net.network import CAModel
...
with tf.Session(config=sess_config) as sess:
    ...
    (output, _) = model.evaluate(input_image_tf, input_mask_tf, config=config, reuse=reuse)
    ...
    for i in range(test_num):
        ...
        cv2.imwrite(os.path.join(config.saving_path, 'input_{:03d}.png'.format(i)), image[(:, :, ::(- 1))].astype(np.uint8))
