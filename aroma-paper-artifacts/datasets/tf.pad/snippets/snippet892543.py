...
def model_graph(features, mode, params):
    ...
    if params.reverse_source:
        src_seq = tf.reverse_sequence(src_seq, seq_dim=1, seq_lengths=features['source_length'])
    ...
    for _ in range(params.num_hidden_layers):
        ...
        cell_e = tf.nn.rnn_cell.DropoutWrapper(cell_e, output_keep_prob=(1.0 - params.dropout), variational_recurrent=params.use_variational_dropout, input_size=params.embedding_size, dtype=tf.float32)
        cell_d = tf.nn.rnn_cell.DropoutWrapper(cell_d, output_keep_prob=(1.0 - params.dropout), variational_recurrent=params.use_variational_dropout, input_size=params.embedding_size, dtype=tf.float32)
    ...
    with tf.variable_scope('encoder'):
        (_, final_state) = tf.nn.dynamic_rnn(cell_enc, src_inputs, features['source_length'], dtype=tf.float32)
    shifted_tgt_inputs = tf.pad(tgt_inputs, [[0, 0], [1, 0], [0, 0]])
    shifted_tgt_inputs = shifted_tgt_inputs[(:, :(- 1), :)]
    with tf.variable_scope('decoder'):
        (outputs, _) = tf.nn.dynamic_rnn(cell_dec, shifted_tgt_inputs, features['target_length'], initial_state=final_state)
    if params.dropout:
        outputs = tf.nn.dropout(outputs, (1.0 - params.dropout))
    if (mode == 'infer'):
        logits = layers.nn.linear(outputs[(:, (- 1), :)], tgt_vocab_size, True, scope='softmax')
