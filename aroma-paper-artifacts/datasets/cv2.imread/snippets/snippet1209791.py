...
if (__name__ == '__main__'):
    ...
    load_name = os.path.join('./models', 'fpn_{}_{}_{}_adv.pth'.format(args.checksession, args.checkepoch, args.checkpoint))
    if (args.net == 'res101'):
    ...
    dataset = roibatchLoader(roidb, ratio_list, ratio_index, args.batch_size, imdb.num_classes, training=False, normalize=False)
    ...
    for i in range(num_images):
        ...
        det_tic = time.time()
        (rois, cls_prob, bbox_pred, _, _, _, _, _) = fpn(im_data, im_info, gt_boxes, num_boxes)
