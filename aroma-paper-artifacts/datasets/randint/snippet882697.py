...
def forward(self, x, target=None, mixup=False, manifold_mixup=False, alpha=None, lam=None, patchup=False, dropblock=False, epoch=None, patchup_type=PatchUpMode.SOFT, k=2, dropblock_all=False):
    ...
    if ((not patchup) and (not dropblock) and (layer_mix == 2) and (layer_mix <= k)):
        ...
        (target_a, target_b, target_reweighted, out, portion) = self.patchup_1(out, target_reweighted, lam=lam_value, patchup_type=patchup_type)
    ...
    if ((not patchup) and (not dropblock) and (layer_mix == 3) and (layer_mix <= k)):
        ...
        (target_a, target_b, target_reweighted, out, portion) = self.patchup_2(out, target_reweighted, lam=lam_value, patchup_type=patchup_type)
    ...
    if ((not patchup) and (not dropblock) and (layer_mix == 4) and (layer_mix <= k)):
        ...
        (target_a, target_b, target_reweighted, out, portion) = self.patchup_3(out, target_reweighted, lam=lam_value, patchup_type=patchup_type)
