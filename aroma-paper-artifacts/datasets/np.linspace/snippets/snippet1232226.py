...
def main():
    ...
    if raw:
        ...
        build_files(raw_data_path=raw_data_path, tokenized_data_path=tokenized_data_path, full_tokenizer=full_tokenizer, num_pieces=num_pieces)
        print('files built')
        ...
        for i in x:
            ...
            for step in range((len(samples) // batch_size)):
                ...
                if (((step + 1) % log_step) == 0):
                    print('now time: {}:{}. Step {} of piece {} of epoch {}, loss {}'.format(datetime.now().hour, datetime.now().minute, ((step + 1) // gradient_accumulation), piece_num, (epoch + 1), (running_loss / log_step)))
                    running_loss = 0
            piece_num += 1
