...
def main():
    ...
    def load_env(env_mode, device_idx):
        if (args.env_type == 'gibson'):
            ...
            return InteractiveNavigateEnv(config_file=config_file, mode=env_mode, action_timestep=args.action_timestep, physics_timestep=args.physics_timestep, random_position=args.random_position, automatic_reset=True, device_idx=device_idx, arena=args.arena)
    ...
    actor_critic = Policy(observation_space=observation_space, action_space=train_envs.action_space, hidden_size=args.hidden_size, cnn_layers_params=cnn_layers_params, initial_stddev=args.action_init_std_dev, min_stddev=args.action_min_std_dev, stddev_anneal_schedule=args.action_std_dev_anneal_schedule, stddev_transform=torch.nn.functional.softplus)
    ...
    if args.eval_only:
        evaluate(args, eval_envs, meta_actor_critic, actor_critic, action_mask_choices, subgoal_mask_choices, subgoal_tolerance, device, writer, update=0, count_steps=0, eval_only=True)
        return
    ...
    if args.use_action_hindsight:
        meta_rollouts_action_hindsight = AsyncRolloutStorage(args.num_steps, train_envs._num_envs, meta_observation_space, subgoal_space, args.hidden_size)
    ...
    for update in range(start_epoch, args.num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                (values, actions, action_log_probs, recurrent_hidden_states) = actor_critic.act(step_observation, rollouts.recurrent_hidden_states[step], rollouts.masks[step], update=update)
            ...
            success_masks = torch.tensor([([float(info['success'])] if (done and ('success' in info)) else [0.0]) for (done, info) in zip(dones, infos)], dtype=torch.float, device=device)
            ...
            collision_steps = torch.tensor([([float(info['collision_step'])] if (done and ('collision_step' in info)) else [0.0]) for (done, info) in zip(dones, infos)], dtype=torch.float, device=device)
            ...
            collision_rewards = torch.tensor([([float(info['collision_reward'])] if ('collision_reward' in info) else [0.0]) for info in infos], dtype=torch.float, device=device)
            ...
            subgoals_done = ((subgoals_achieved | (current_subgoals_steps == args.time_scale)) | (1.0 - masks).byte())
            subgoals_done = subgoals_done.float()
            subgoals_achieved = subgoals_achieved.float()
            penalty_prob_th = 0.5
            current_subgoals_penalty = (((args.subgoal_failed_penalty * subgoals_done) * (1.0 - subgoals_achieved)) * (torch.rand(train_envs._num_envs, 1, device=device) < penalty_prob_th).float())
            ...
            if args.use_action_hindsight:
                ...
                (_, hindsight_subgoal_log_probs, _, _, _, _) = meta_actor_critic.evaluate_actions(meta_step_observation, meta_rollouts.recurrent_hidden_states[(meta_rollouts.valid_steps, meta_rollouts.env_indices)], meta_rollouts.masks[(meta_rollouts.valid_steps, meta_rollouts.env_indices)], hindsight_subgoals, current_action_mask_indices)
                hindsight_subgoals = ((subgoals_achieved * original_subgoals) + ((1 - subgoals_achieved) * hindsight_subgoals))
                hindsight_subgoal_log_probs = ((subgoals_achieved * current_subgoal_log_probs) + ((1 - subgoals_achieved) * hindsight_subgoal_log_probs))
                meta_rollouts_action_hindsight.insert(subgoals_done, batch, next_meta_recurrent_hidden_states, hindsight_subgoals, hindsight_subgoal_log_probs, current_action_mask_indices, current_action_mask_log_probs, current_meta_values, (current_subgoals_cumulative_rewards + current_subgoals_penalty), masks)
            ...
            rollouts.insert(batch, recurrent_hidden_states, actions, action_log_probs, values, intrinsic_reward, (1 - subgoals_done))
        ...
        if args.use_action_hindsight:
            meta_rollouts_action_hindsight.compute_returns(next_meta_value, args.use_gae, args.meta_gamma, args.tau)
