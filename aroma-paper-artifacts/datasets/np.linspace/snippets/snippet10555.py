...
if (__name__ == '__main__'):
    ...
    trajectory = ntm.evolve(initial_conditions=initial_conditions, network=network, activity_rule=activity_rule, timesteps=timesteps)
    activities = ntm.get_activities_over_time_as_list(trajectory)
    activities = np.array(activities)
    ntm.poincare_plot(activities=[activities[(:, i)] for i in range(num_nodes)], timesteps=int((timesteps / 2)), xlabel='Population', ylabel='Population(t + 1)', xlim=(0.25, 0.75), ylim=(0.8, 1.01))
