...
def main(argv):
    ...
    if hist:
        ...
        results = aranet4.client.get_all_records(mac_address=device_mac, entry_filter={'last': limit})
    else:
        ...
        results = [{'time': t.timestamp(), 'id': 0, 'temperature': current.temperature, 'pressure': current.pressure, 'humidity': current.humidity, 'co2': current.co2}]
    pts = []
    print('Sending history to InfluxDB...')
    for r in results:
        strtim = datetime.datetime.utcfromtimestamp(r['time']).strftime('%Y-%m-%dT%H:%M:%SZ')
        t = r['temperature']
        p = r['pressure']
        h = r['humidity']
        c = r['co2']
