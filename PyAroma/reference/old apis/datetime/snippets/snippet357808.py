...
def sunrise(observer: Observer, date: Optional[datetime.date]=None, tzinfo: Union[(str, datetime.tzinfo)]=datetime.timezone.utc) -> datetime.datetime:
    ...
    if (date is None):
        ...
        date = date.date()
    try:
        tot = time_of_transit(observer, date, (90.0 + SUN_APPARENT_RADIUS), SunDirection.RISING).astimezone(tzinfo)
        tot_date = tot.date()
        if (tot_date != date):
            if (tot_date < date):
                ...
                delta = datetime.timedelta(days=(- 1))
            new_date = (date + delta)
            tot = time_of_transit(observer, new_date, (90.0 + SUN_APPARENT_RADIUS), SunDirection.RISING).astimezone(tzinfo)
            tot_date = tot.date()
            if (tot_date != date):
