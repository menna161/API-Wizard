...
def main(wf):
    ...
    if wf.update_available:
        wf.add_item('A newer version is available', '↩ to install update', icon='update-available.png', arg='update', valid=True)
    query = None
    if len(wf.args):
        query = wf.args[0]
    ...
    if ((not use_google) and (not use_exchange)):
        ...
        wf.send_feedback()
        return
    log.debug('Max Age: %i  Cache Age Google:  %i   Exchange: %i', cache_time, wf.cached_data_age(google_cache_key), wf.cached_data_age(exchange_cache_key))
    ...
    if use_exchange:
        if exchange_fresh:
            ...
            if (not is_running('update_exchange')):
                cmd = ['/usr/bin/python', wf.workflowfile('query_exchange.py'), start_outlook.strftime('%Y-%m-%d-%H:%M:%S'), end_outlook.strftime('%Y-%m-%d-%H:%M:%S'), str(date_offset)]
    ...
    if use_google:
        ...
        if google_fresh:
            ...
            if (not is_running('update_google')):
                cmd = ['/usr/bin/python', wf.workflowfile('query_google.py'), start_google, stop_google, str(date_offset)]
        ...
        if (google_events is None):
            ...
            try:
                conn.request('HEAD', '/')
                wf.add_item('Unable to connect to Google', 'Authorization or Connection error - use tc to reauthorize', icon='img/disclaimer.png')
                ...
                wf.add_item('Unable to connect to Google', 'Check your internet connection or proxy settings', icon='img/disclaimer.png')
            google_events = []
        else:
    ...
    if showing_cached_data:
        ...
        first_menu_entry.subtitle += ((' query time: ' + '{:.1f}'.format(action_elapsed_time)) + ' seconds')
    wf.send_feedback()
