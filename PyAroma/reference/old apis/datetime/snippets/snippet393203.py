...
def bootstrap():
    ...
    if click.confirm('Should I upload example data to Postgres (overwriting "feast_driver_hourly_stats" table)?', default=True):
        db_connection = psycopg2.connect(dbname=postgres_database, host=postgres_host, port=int(postgres_port), user=postgres_user, password=postgres_password, options=f'-c search_path={postgres_schema}')
        with db_connection as conn, conn.cursor() as cur:
            cur.execute('DROP TABLE IF EXISTS "feast_driver_hourly_stats"')
        df_to_postgres_table(config=PostgreSQLConfig(host=postgres_host, port=int(postgres_port), database=postgres_database, db_schema=postgres_schema, user=postgres_user, password=postgres_password), df=driver_df, table_name='feast_driver_hourly_stats')
