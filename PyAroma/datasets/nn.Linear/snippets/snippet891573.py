...
def __init__(self, sent_dim, concept_dim, relation_dim, concept_num, relation_num, qas_encoded_dim, pretrained_concept_emd, pretrained_relation_emd, lstm_dim, lstm_layer_num, device, graph_hidden_dim, graph_output_dim, dropout=0.1, bidirect=True, num_random_paths=None, path_attention=True, qa_attention=True):
    ...
    self.lstm = nn.LSTM(input_size=((self.graph_output_dim + self.concept_dim) + self.relation_dim), hidden_size=lstm_dim, num_layers=lstm_layer_num, bidirectional=bidirect, dropout=dropout)
    if bidirect:
        self.lstm_dim = (lstm_dim * 2)
    else:
        self.lstm_dim = lstm_dim
    self.qas_encoder = nn.Sequential(nn.Linear(((2 * (self.graph_output_dim + self.concept_dim)) + sent_dim), (self.qas_encoded_dim * 2)), nn.Dropout(dropout), nn.LeakyReLU(), nn.Linear((self.qas_encoded_dim * 2), self.qas_encoded_dim), nn.Dropout(dropout), nn.LeakyReLU())
    ...
    self.graph_encoder = GCN_Encoder(self.concept_dim, self.graph_hidden_dim, self.graph_output_dim, pretrained_concept_emd=None, concept_emd=self.concept_emd)
