...
def __call__(self, y_true, y_pred):
    ...
    for idx in range(num):
        ...
        match(threshold=self.threshold, truths=truths, priors=priors, variances=self.variance, labels=labels, loc_t=loc_t, conf_t=conf_t)
    ...
    loss_c = (log_sum_exp(batch_conf) - tf.gather(params=batch_conf, indices=tf.reshape(conf_t, shape=((- 1), 1)), batch_dims=1))
