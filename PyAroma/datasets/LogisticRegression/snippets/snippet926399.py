...
def test_extractor(html):
    ...
    model_attrs = {'C': 1.0, 'class_weight': None, 'classes_': [0, 1], 'coef_': [[0.00501458328421719, (- 0.0006331822163374379), (- 0.6699789320373452), 0.026069227973339763, (- 1.5552477377277252), 0.02980432745983307, (- 0.965575689884716), 0.019509367890934326, (- 0.35692924115362307)]], 'dual': False, 'fit_intercept': True, 'intercept_': [(- 1.2071425754440765)], 'intercept_scaling': 1, 'max_iter': 100, 'multi_class': 'ovr', 'n_iter_': [12], 'n_jobs': 1, 'penalty': 'l2', 'solver': 'liblinear', 'tol': 0.0001, 'warm_start': False}
    ...
    for (k, v) in model_attrs.items():
        if isinstance(v, list):
            ...
            setattr(model, k, v)
    extractor = Extractor(blockifier, features=features, model=model, to_extract='content', prob_threshold=prob_threshold)
