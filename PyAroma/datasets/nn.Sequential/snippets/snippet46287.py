...
def __init__(self, n_part1, n_part2, t_stride, t_kernel, t_padding, drop=0.2, layer1=False, nmp=False):
    super().__init__()
    self.time_conv = nn.Sequential(nn.Conv2d(n_part1, n_part1, kernel_size=(t_kernel, 1), stride=(t_stride, 1), padding=(t_padding, 0), bias=True), nn.BatchNorm2d(n_part1), nn.Dropout(drop, inplace=True))
    if (nmp == True):
