...
def __init__(self, previous, num_classes, num_blocks_per_stage=None, network_props=None, deep_supervision=False, upscale_logits=False):
    ...
    for (i, s) in enumerate(np.arange(num_stages)[::(- 1)]):
        ...
        self.stages.append(StackedConvLayers((2 * features_skip), features_skip, previous_stage_conv_op_kernel_size[s], self.props, num_blocks_per_stage[i]))
