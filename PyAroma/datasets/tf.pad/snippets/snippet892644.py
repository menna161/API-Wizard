...
def _get_inference_fn(model_fns, features):
    ...
    def inference_fn(inputs, state):
        local_features = {'source': features['source'], 'source_length': features['source_length'], 'context': features['context'], 'context_length': features['context_length'], 'target': tf.pad(inputs[(:, 1:)], [[0, 0], [0, 1]]), 'target_length': tf.fill([tf.shape(inputs)[0]], tf.shape(inputs)[1])}
        outputs = []
        next_state = []
