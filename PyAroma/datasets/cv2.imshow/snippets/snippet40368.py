...
def showpoints(xyz, c_gt=None, c_pred=None, waittime=0, showrot=False, magnifyBlue=0, freezerot=False, background=(0, 0, 0), normalizecolor=True, ballradius=10):
    ...
    def render():
        ...
        if (not freezerot):
            ...
            xangle = 0
        rotmat = rotmat.dot(np.array([[1.0, 0.0, 0.0], [0.0, np.cos(xangle), (- np.sin(xangle))], [0.0, np.sin(xangle), np.cos(xangle)]]))
        if (not freezerot):
            yangle = (((mousex - 0.5) * np.pi) * 1.2)
        else:
            yangle = 0
        rotmat = rotmat.dot(np.array([[np.cos(yangle), 0.0, (- np.sin(yangle))], [0.0, 1.0, 0.0], [np.sin(yangle), 0.0, np.cos(yangle)]]))
        rotmat *= zoom
        nxyz = (xyz.dot(rotmat) + [(showsz / 2), (showsz / 2), 0])
        ixyz = nxyz.astype('int32')
        show[:] = background
        dll.render_ball(ct.c_int(show.shape[0]), ct.c_int(show.shape[1]), show.ctypes.data_as(ct.c_void_p), ct.c_int(ixyz.shape[0]), ixyz.ctypes.data_as(ct.c_void_p), c0.ctypes.data_as(ct.c_void_p), c1.ctypes.data_as(ct.c_void_p), c2.ctypes.data_as(ct.c_void_p), ct.c_int(ballradius))
        if (magnifyBlue > 0):
