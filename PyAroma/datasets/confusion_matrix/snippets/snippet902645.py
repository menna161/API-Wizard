...
def evaluate(epoch_predicts, epoch_labels, vocab, threshold=0.5, top_k=None):
    ...
    for (i, label) in id2label.items():
        (precision_dict[label], recall_dict[label], fscore_dict[label]) = _precision_recall_f1(right_count_list[i], predicted_count_list[i], gold_count_list[i])
    ...
    return {'precision': precision_micro, 'recall': recall_micro, 'micro_f1': micro_f1, 'macro_f1': macro_f1}
