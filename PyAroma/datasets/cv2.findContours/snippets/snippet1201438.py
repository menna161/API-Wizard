...
if (__name__ == '__main__'):
    ...
    for _file in tqdm(files[:]):
        ...
        final_mask = rasterDriver.Create('', 900, 900, 1, gdal.GDT_Byte)
        ...
        final_mask = rasterDriver.Create(out_path, 900, 900, 2, gdal.GDT_Byte)
        final_mask.SetGeoTransform(tileGeoTransformationParams)
        final_mask.SetProjection(projection)
        tempTile = final_mask.GetRasterBand(1)
        tempTile.Fill(0)
    ...
    for _file in tqdm(files[:]):
        for nadir_name in all_nadirs[:]:
            ...
            final_image = rasterDriver.Create(out_path, 900, 900, 3, gdal.GDT_UInt16)
