...
def network(im, im_deformed, bn_training, detailed_summary=False):
    ...
    with tf.variable_scope('DownSampling_R1'):
        margin_r1 = 49
        images_r1 = tf.concat([im[(:, margin_r1:(- margin_r1), margin_r1:(- margin_r1), margin_r1:(- margin_r1), :)], im_deformed[(:, margin_r1:(- margin_r1), margin_r1:(- margin_r1), margin_r1:(- margin_r1), :)]], axis=(- 1))
    with tf.variable_scope('R4'):
        ...
        conv6_r4 = tfu.layers.conv3d_transpose(conv5_r4, 40, [3, 3, 3], strides=(2, 2, 2), scope='conv6_R4', use_bias=False, initializer='trilinear', **common)
        conv7_r4 = tfu.layers.conv3d(conv6_r4, 45, [3, 3, 3], padding='same', activation='ReLu', bn_training=bn_training, scope='conv7_R4')
        conv8_r4 = tfu.layers.conv3d_transpose(conv7_r4, 50, [3, 3, 3], padding='valid', activation='ReLu', strides=(2, 2, 2), bn_training=bn_training, scope='conv8_R4', use_bias=False, initializer='trilinear')
    ...
    with tf.variable_scope('R2'):
        ...
        conv6_r2 = tfu.layers.conv3d_transpose(conv5_r2, 40, [3, 3, 3], strides=(2, 2, 2), scope='conv6_R2', use_bias=False, initializer='trilinear', **common)
