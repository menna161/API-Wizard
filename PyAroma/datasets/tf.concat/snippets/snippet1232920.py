...
def bidirectional_dynamic_rnn_2inputs(cell_fw, cell_bw, inputs_fw, inputs_bw, sequence_length=None, initial_state_fw=None, initial_state_bw=None, dtype=None, parallel_iterations=None, swap_memory=False, time_major=False, scope=None):
    ...
    with vs.variable_scope((scope or 'bidirectional_rnn')):
        with vs.variable_scope('fw') as fw_scope:
            (output_fw, output_state_fw) = rnn.dynamic_rnn(cell=cell_fw, inputs=inputs_fw, sequence_length=sequence_length, initial_state=initial_state_fw, dtype=dtype, parallel_iterations=parallel_iterations, swap_memory=swap_memory, time_major=time_major, scope=fw_scope)
        if (not time_major):
            time_dim = 1
            batch_dim = 0
        else:
        ...
        with vs.variable_scope('bw') as bw_scope:
            ...
            (tmp, output_state_bw) = rnn.dynamic_rnn(cell=cell_bw, inputs=inputs_reverse, sequence_length=sequence_length, initial_state=initial_state_bw, dtype=dtype, parallel_iterations=parallel_iterations, swap_memory=swap_memory, time_major=time_major, scope=bw_scope)
    if (type(tmp) == tuple):
